...
{
    esp_http_client_t *client = (esp_http_client_t *)parser->data;
...
esp_http_client_handle_t esp_http_client_init(const esp_http_client_config_t *config)
{
    ...
    _success = (
                   (client                    = (esp_http_client_handle_t)calloc(1, sizeof(esp_http_client_t)))         &&
                   (client->parser            = (http_parser *)calloc(1, sizeof(struct http_parser)))                   &&
                   (client->parser_settings   = (http_parser_settings *)calloc(1, sizeof(struct http_parser_settings))) &&
                   (client->auth_data         = (esp_http_auth_data_t *)calloc(1, sizeof(esp_http_auth_data_t)))        &&
                   (client->request           = (esp_http_data_t *)calloc(1, sizeof(esp_http_data_t)))                  &&
                   (client->request->headers  = http_header_init())                                                     &&
                   (client->request->buffer   = (esp_http_buffer_t *)calloc(1, sizeof(esp_http_buffer_t)))              &&
                   (client->response          = (esp_http_data_t *)calloc(1, sizeof(esp_http_data_t)))                  &&
                   (client->response->headers = http_header_init())                                                     &&
                   (client->response->buffer  = (esp_http_buffer_t *)calloc(1, sizeof(esp_http_buffer_t)))
               );
    ...
    _success = (
                   (client->request->buffer->data  = (char *)malloc(client->buffer_size_tx))  &&
                   (client->response->buffer->data = (char *)malloc(client->buffer_size_rx))
               );
...
esp_err_t esp_http_client_set_url(esp_http_client_handle_t client, const char *url)
{
    ...
    if (old_host && client->connection_info.host
            && strcasecmp(old_host, (const char *)client->connection_info.host) != 0) {
...
static esp_err_t esp_http_client_send_post_data(esp_http_client_handle_t client)
{
    ...
    wret = esp_http_client_write(client, client->post_data + client->data_written_index, client->data_write_left);
    if (wret < 0) {
        return wret;
    }
    client->data_write_left -= wret;
